...
describe('describeUnitReady', () => {
  it('returns found=true with stars/gear/relic for an owned unit', () => {
  ...
  it('emits a "next relic" hint when at gear 13 and relic < 9', () => {
    const r = describeUnitReady(player, 'JEDIMASTERKENOBI');
    expect(r.gearLevel).toBe(13);
    expect(r.nextStepHint).toMatch(/Relic\s*9/);
  });

  it('returns "fully geared" when at gear 13 and relic 9', () => {
    const r = describeUnitReady(player, 'GLREY');
