...
export function generateOffenseStrategyHtml(
  ...
  const opponentStatsMap = opponentRoster ? buildCharacterStatsMap(opponentRoster) : new Map<string, CharacterStatEntry>();
  ...
  const getCharacterStats = (baseId: string, isOffense: boolean): CharacterStatEntry | null => {
    if (isOffense) {
  ...
  logger.info(`[Offense Image] Generated ${visibleOffense.length} battle row(s) for HTML`);
  
  // Verify battleRows doesn't contain duplicates by checking for duplicate battle numbers
  const battleNumberMatches = battleRows.match(/Battle \d+/g);
  if (battleNumberMatches) {
    const uniqueBattles = new Set(battleNumberMatches);
    if (battleNumberMatches.length !== uniqueBattles.size) {
      logger.error(`[Offense Image] WARNING: Found duplicate battle numbers in HTML! Total: ${battleNumberMatches.length}, Unique: ${uniqueBattles.size}`);
    } else {
      logger.info(`[Offense Image] Verified: All ${battleNumberMatches.length} battle numbers are unique`);
    }
  }
...
<body>
  ...
  // Final verification: count battle-row divs in the HTML string
  const battleRowCount = (html.match(/<div class="battle-row">/g) || []).length;
  if (battleRowCount !== visibleOffense.length) {
    logger.error(`[Offense Image] CRITICAL: HTML contains ${battleRowCount} battle-row divs but expected ${visibleOffense.length}!`);
  } else {
    logger.info(`[Offense Image] Verified: HTML contains exactly ${battleRowCount} battle-row div(s)`);
  }
