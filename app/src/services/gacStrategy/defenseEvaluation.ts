...
export async function evaluateRosterForDefense(
  ...
  archetypeValidation?: ArchetypeValidationInfo;
    ...
    logger.info(
      `Fetching top defense squads for ${format} format${seasonId ? ` (season: ${seasonId})` : ' (no season specified)'}`
    );
    ...
    // Log squad sizes to verify format
    if (topDefenseSquadsByCount.length > 0) {
      const sampleSquad = topDefenseSquadsByCount[0];
      const squadSize = 1 + sampleSquad.members.length;
      const expectedSize = format === '3v3' ? 3 : 5;
      if (squadSize !== expectedSize) {
        logger.warn(
          `Format mismatch detected: Expected ${format} (${expectedSize} units) but top defense squads have ${squadSize} units. ` +
          `Sample: ${sampleSquad.leader.baseId} + [${sampleSquad.members.map(m => m.baseId).join(', ')}]`
        );
      } else {
        logger.info(
          `Verified format: Top defense squads match ${format} format (${squadSize} units per squad)`
        );
      }
    }
