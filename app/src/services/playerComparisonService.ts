...
import { getCharacterPortraitUrl, initializeCharacterPortraits } from '../config/characterPortraits';
...
export class PlayerComparisonService {
   ...
   * Includes all characters from both players' rosters, not just GLs.
   */
  private buildCharacterImageCache(
    p1: SwgohGgFullPlayerResponse,
    p2: SwgohGgFullPlayerResponse
  ): Map<string, string> {
    const cache = new Map<string, string>();
    
    // Collect all unique character baseIds from both players
    const allCharacterIds = new Set<string>();
    
    // Add all characters from player 1 (only characters, not ships)
    for (const unit of p1.units) {
      if (unit.data && unit.data.base_id && unit.data.combat_type === 1) {
        allCharacterIds.add(unit.data.base_id);
      }
    }
    
    // Add all characters from player 2 (only characters, not ships)
    ...
    // For all characters, get portrait URLs from cache/fallback
    for (const baseId of allCharacterIds) {
      const url = getCharacterPortraitUrl(baseId);
      if (url) {
        cache.set(baseId, url);
      }
    }
    
    logger.debug(`Built character image cache with ${cache.size} character portraits from ${allCharacterIds.size} unique characters`);
    return cache;
  }
  ...
  async generateComparisonImage(
    ...
    // Ensure portrait cache is loaded (in case it was updated)
    await initializeCharacterPortraits();
    
    const browser = await this.getBrowser();
    const page = await browser.newPage();

    try {
      // Build character image cache from existing portraits and player data
      const characterImageCache = this.buildCharacterImageCache(p1, p2);
