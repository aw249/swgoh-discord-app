...
export class GacStrategyService {
    ...
    if (!this.browser) {
      ...
      const launchOptions: Parameters<typeof puppeteer.launch>[0] = {
        ...
        args: [
          ...
          '--no-first-run',
          '--no-zygote',
          '--single-process', // Helps with ARM64 stability
      ...
      if (executablePath) {
        ...
        logger.info(`[Browser] Using Chromium executable: ${executablePath}`);
      }
      
      this.browser = await puppeteer.launch(launchOptions);
      logger.info(`[Browser] Puppeteer browser launched successfully`);
    }
    return this.browser;
  }
  ...
  async generateSplitStrategyImages(
    ...
    try {
      ...
      // Set initial viewport - we'll adjust height dynamically after content loads
      await defensePage.setViewport({ width: defenseWidth, height: 1000, deviceScaleFactor: 2 });
      logger.info(`[Defense Image] Generating defense HTML...`);
      ...
      // Verify HTML is defense HTML (should contain "DEFENSE" or "Defense" in title/header)
      const defenseHtmlPreview = defenseHtml.substring(0, 200);
      logger.info(`[Defense Image] Defense HTML preview (first 200 chars): ${defenseHtmlPreview.replace(/\n/g, ' ').substring(0, 150)}...`);
      logger.info(`[Defense Image] Defense HTML length: ${defenseHtml.length} chars, contains 'DEFENSE' or 'Defense': ${defenseHtml.includes('DEFENSE') || defenseHtml.includes('Defense')}`);
      logger.info(`[Defense Image] Setting page content and waiting for load...`);
      await defensePage.setContent(defenseHtml, { waitUntil: 'networkidle0' });
      
      // Wait for rendering to complete (ARM64 Chromium may need extra time)
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // Verify the page actually has defense content
      const defensePageContent = await defensePage.evaluate(() => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const doc: any = (globalThis as any).document;
        const header = doc.querySelector('.header');
        const headerText = header ? header.textContent : null;
        const container = doc.querySelector('.container');
        const squadRows = container ? container.querySelectorAll('.squad-row') : null;
        return {
          headerText,
          hasDefenseHeader: headerText ? headerText.includes('DEFENSE') : false,
          squadCount: squadRows ? squadRows.length : 0,
          hasOffenseHeader: headerText ? headerText.includes('OFFENSE') : false
        };
      });
      logger.info(`[Defense Image] Page verification after setContent:`);
      logger.info(`  - Header text: ${defensePageContent.headerText}`);
      logger.info(`  - Has DEFENSE header: ${defensePageContent.hasDefenseHeader}`);
      logger.info(`  - Has OFFENSE header (ERROR if true): ${defensePageContent.hasOffenseHeader}`);
      logger.info(`  - Squad count: ${defensePageContent.squadCount} (expected: ${balancedDefense.length})`);
      
      if (defensePageContent.hasOffenseHeader) {
        logger.error(`[Defense Image] CRITICAL ERROR: Defense page contains OFFENSE header! This indicates HTML mixing.`);
      }
      if (defensePageContent.squadCount !== balancedDefense.length) {
        logger.warn(`[Defense Image] WARNING: Expected ${balancedDefense.length} defense squads but found ${defensePageContent.squadCount} in DOM`);
      }
      
      // Get container bounds and adjust viewport dynamically
      const defenseBounds = await defensePage.evaluate(() => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const doc: any = (globalThis as any).document;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        const bodyPaddingBottom = parseInt(bodyStyle.paddingBottom || '0', 10);
        return {
          x: Math.round(rect.x),
          y: Math.round(rect.y),
          width: Math.round(rect.width),
          height: Math.round(rect.height),
          totalHeight: Math.round(rect.height + rect.y + bodyPaddingBottom)
        };
      });
      
      if (defenseBounds) {
        // Adjust viewport height to match actual content height
        const viewportHeight = Math.max(1000, defenseBounds.totalHeight + 100);
        await defensePage.setViewport({ 
          width: defenseWidth, 
          height: viewportHeight, 
          deviceScaleFactor: 2 
        });
        logger.info(`[Defense Image] Adjusted viewport height to ${viewportHeight}px to match content`);
        
        // Wait for viewport change to take effect
        await new Promise(resolve => setTimeout(resolve, 200));
        
        // Re-get bounds after viewport adjustment
        const finalDefenseBounds = await defensePage.evaluate(() => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const doc: any = (globalThis as any).document;
          const container = doc.querySelector('.container');
          if (!container) return null;
        ...
        // Final verification before screenshot
        const finalDefenseCheck = await defensePage.evaluate(() => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const doc: any = (globalThis as any).document;
          const header = doc.querySelector('.header');
          return header ? header.textContent : null;
        });
        logger.info(`[Defense Image] Final header check before screenshot: ${finalDefenseCheck}`);
        if (finalDefenseCheck && finalDefenseCheck.includes('OFFENSE')) {
          logger.error(`[Defense Image] CRITICAL: Defense page header shows OFFENSE content right before screenshot!`);
        }
        
        logger.info(`[Defense Image] Taking screenshot...`);
        if (finalDefenseBounds) {
          defenseImage = await defensePage.screenshot({ 
            type: 'png', 
            clip: {
              x: finalDefenseBounds.x,
              y: finalDefenseBounds.y,
              width: finalDefenseBounds.width,
              height: finalDefenseBounds.height
            }
          }) as Buffer;
          logger.info(`[Defense Image] Screenshot taken with clip: ${finalDefenseBounds.width}x${finalDefenseBounds.height} at (${finalDefenseBounds.x}, ${finalDefenseBounds.y})`);
        } else {
          defenseImage = await defensePage.screenshot({ type: 'png', fullPage: true }) as Buffer;
          logger.warn(`[Defense Image] Container not found after viewport adjustment, used fullPage screenshot`);
        }
      } else {
        logger.warn(`[Defense Image] Container not found, using fullPage screenshot`);
        defenseImage = await defensePage.screenshot({ type: 'png', fullPage: true }) as Buffer;
      }
      logger.info(`[Defense Image] Screenshot complete (${defenseImage.length} bytes)`);
    } finally {
    ...
    try {
      ...
      // Set initial viewport - we'll adjust height dynamically after content loads
      await offensePage.setViewport({ width: offenseWidth, height: 1000, deviceScaleFactor: 2 });
      logger.info(`[Offense Image] Generating offense HTML...`);
      ...
      // Verify HTML is offense HTML (should contain "OFFENSE" or "Offense" in title/header)
      const offenseHtmlPreview = offenseHtml.substring(0, 200);
      logger.info(`[Offense Image] Offense HTML preview (first 200 chars): ${offenseHtmlPreview.replace(/\n/g, ' ').substring(0, 150)}...`);
      logger.info(`[Offense Image] Offense HTML length: ${offenseHtml.length} chars, contains 'OFFENSE' or 'Offense': ${offenseHtml.includes('OFFENSE') || offenseHtml.includes('Offense')}`);
      logger.info(`[Offense Image] Setting page content and waiting for load...`);
      await offensePage.setContent(offenseHtml, { waitUntil: 'networkidle0' });
      
      // Verify the page actually has offense content
      const offensePageContent = await offensePage.evaluate(() => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const doc: any = (globalThis as any).document;
        const header = doc.querySelector('.header');
        const headerText = header ? header.textContent : null;
        const container = doc.querySelector('.container');
        const battleRows = container ? container.querySelectorAll('.battle-row') : null;
        return {
          headerText,
          hasOffenseHeader: headerText ? headerText.includes('OFFENSE') : false,
          battleCount: battleRows ? battleRows.length : 0,
          hasDefenseHeader: headerText ? headerText.includes('DEFENSE') : false
        };
      });
      logger.info(`[Offense Image] Page verification after setContent:`);
      logger.info(`  - Header text: ${offensePageContent.headerText}`);
      logger.info(`  - Has OFFENSE header: ${offensePageContent.hasOffenseHeader}`);
      logger.info(`  - Has DEFENSE header (ERROR if true): ${offensePageContent.hasDefenseHeader}`);
      logger.info(`  - Battle count: ${offensePageContent.battleCount} (expected: ${balancedOffense.length})`);
      
      if (offensePageContent.hasDefenseHeader) {
        logger.error(`[Offense Image] CRITICAL ERROR: Offense page contains DEFENSE header! This indicates HTML mixing.`);
      }
      if (offensePageContent.battleCount !== balancedOffense.length) {
        logger.warn(`[Offense Image] WARNING: Expected ${balancedOffense.length} battles but found ${offensePageContent.battleCount} in DOM`);
      }
      
      // Wait for rendering to complete (ARM64 Chromium may need extra time)
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // Get the actual container height and adjust viewport to match content
      const containerBounds = await offensePage.evaluate(() => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const doc: any = (globalThis as any).document;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const win: any = (globalThis as any).window;
        ...
        if (!container) return null;
        const rect = container.getBoundingClientRect();
        // Also get body padding to account for it
        const bodyStyle = win.getComputedStyle(doc.body);
        const bodyPaddingTop = parseInt(bodyStyle.paddingTop || '0', 10);
        const bodyPaddingBottom = parseInt(bodyStyle.paddingBottom || '0', 10);
        return {
          x: Math.round(rect.x),
          y: Math.round(rect.y),
      ...
      if (containerBounds) {
        // Adjust viewport height to match actual content height (with some padding for safety)
        const viewportHeight = Math.max(1000, containerBounds.totalHeight + 100);
        await offensePage.setViewport({ 
          width: offenseWidth, 
          height: viewportHeight, 
          deviceScaleFactor: 2 
        });
        logger.info(`[Offense Image] Adjusted viewport height to ${viewportHeight}px to match content`);
        
        // Wait for viewport change to take effect
        await new Promise(resolve => setTimeout(resolve, 200));
        
        // Re-get bounds after viewport adjustment
        const finalBounds = await offensePage.evaluate(() => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const doc: any = (globalThis as any).document;
          const container = doc.querySelector('.container');
          if (!container) return null;
        ...
        // Final verification before screenshot
        const finalOffenseCheck = await offensePage.evaluate(() => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const doc: any = (globalThis as any).document;
          const header = doc.querySelector('.header');
          const battleRows = doc.querySelectorAll('.battle-row');
          return {
            headerText: header ? header.textContent : null,
            battleCount: battleRows.length
          };
        });
        logger.info(`[Offense Image] Final check before screenshot:`);
        logger.info(`  - Header: ${finalOffenseCheck.headerText}`);
        logger.info(`  - Battle count: ${finalOffenseCheck.battleCount} (expected: ${balancedOffense.length})`);
        if (finalOffenseCheck.headerText && finalOffenseCheck.headerText.includes('DEFENSE')) {
          logger.error(`[Offense Image] CRITICAL: Offense page header shows DEFENSE content right before screenshot!`);
        }
        if (finalOffenseCheck.battleCount !== balancedOffense.length) {
          logger.error(`[Offense Image] CRITICAL: Expected ${balancedOffense.length} battles but found ${finalOffenseCheck.battleCount} right before screenshot!`);
        }
        
        logger.info(`[Offense Image] Taking screenshot...`);
        if (finalBounds) {
          offenseImage = await offensePage.screenshot({ 
            type: 'png', 
            clip: {
              x: finalBounds.x,
              y: finalBounds.y,
          ...
          logger.info(`[Offense Image] Screenshot taken with clip: ${finalBounds.width}x${finalBounds.height} at (${finalBounds.x}, ${finalBounds.y})`);
        } else {
          // Fallback to fullPage if container not found
          offenseImage = await offensePage.screenshot({ type: 'png', fullPage: true }) as Buffer;
          logger.warn(`[Offense Image] Container not found after viewport adjustment, used fullPage screenshot`);
        }
      } else {
        // Fallback to fullPage if container not found
        logger.warn(`[Offense Image] Container not found, using fullPage screenshot`);
        offenseImage = await offensePage.screenshot({ type: 'png', fullPage: true }) as Buffer;
      }
      logger.info(`[Offense Image] Screenshot complete (${offenseImage.length} bytes)`);
    } finally {
