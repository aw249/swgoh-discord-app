...
export class GacService {
  // Cache bracket data for 15 minutes to support fast autocomplete responses
  // Extended TTL to reduce fetch frequency after app reloads
  private readonly bracketCache: Map<string, CachedBracketData> = new Map();
  private readonly cacheTTL = 15 * 60 * 1000; // 15 minutes in milliseconds
  ...
  /**
   * Get live bracket data quickly for autocomplete purposes.
   * Uses super-lightweight fetch that skips Comlink refresh and Top 80 GP calculation.
   * Also accepts expired cache if fresh data isn't available quickly.
   */
  async getLiveBracketForAutocomplete(allyCode: string, useCache: boolean = true): Promise<LiveBracketData> {
    // Check cache first if enabled (even if expired, it's better than nothing for autocomplete)
    if (useCache) {
      const cached = this.bracketCache.get(allyCode);
      if (cached) {
        const age = Date.now() - cached.timestamp;
        // Use cache if fresh (within TTL) or if expired but less than 1 hour old (better than nothing)
        if (age < this.cacheTTL || age < 60 * 60 * 1000) {
          logger.debug(`Using cached bracket data for autocomplete for ${allyCode} (age: ${Math.round(age / 1000)}s)`);
          return cached.data;
        }
      }
    }

    // Fetch fresh data using super-lightweight approach (swgoh.gg only, no Comlink refresh)
    const bracketData = await this.apiClient.getBracketForAutocomplete(allyCode);
