...
export class CombinedApiClient {
   ...
   */
  /**
   * Get bracket data quickly for autocomplete - prefers Comlink API (faster) over swgoh.gg scraping.
   * Skips Top 80 GP calculation and uses lightweight refresh with timeout.
   */
  async getBracketForAutocomplete(allyCode: string): Promise<LiveBracketData> {
    const normalizedAllyCode = allyCode.replace(/-/g, '');

    let bracketMeta: GacBracketData | null = null;
    let players: GacBracketPlayer[] = [];
    let isRealTime = false;

    // Step 1: Try Comlink first (fast API) if available
    if (await this.isComlinkAvailable()) {
      try {
        // Try lightweight Comlink discovery (skips ally code enrichment for speed)
        bracketMeta = await Promise.race([
          this.discoverBracketViaComlinkLightweight(normalizedAllyCode),
          new Promise<GacBracketData | null>((resolve) =>
            setTimeout(() => resolve(null), 1000) // 1 second timeout for discovery
          )
        ]);

        if (bracketMeta) {
          // If we found bracket via Comlink, try to quickly enrich with ally codes (parallel, with timeout)
          // Only fetch ally codes for players that don't have them (ally_code === 0)
          const playersNeedingAllyCodes = bracketMeta.bracket_players.filter(p => !p.ally_code || p.ally_code === 0);
          
          if (playersNeedingAllyCodes.length > 0) {
            // Fetch ally codes in parallel with a short timeout (only for autocomplete)
            try {
              const allyCodePromises = playersNeedingAllyCodes.map(async (player) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const playerId = (player as any).player_id;
                if (!playerId) return player;
                
                try {
                  const playerData = await Promise.race([
                    this.comlinkClient.getPlayerById(playerId),
                    new Promise<never>((_, reject) =>
                      setTimeout(() => reject(new Error('Timeout')), 300) // 300ms per player
                    )
                  ]);
                  
                  return {
                    ...player,
                    ally_code: parseInt(playerData.allyCode, 10) || 0,
                  };
                } catch (err) {
                  // Timeout or error - keep player with ally_code 0
                  return player;
                }
              });
              
              const enrichedPlayers = await Promise.all(allyCodePromises);
              
              // Replace players in bracket_players with enriched versions
              players = bracketMeta.bracket_players.map(p => {
                const enriched = enrichedPlayers.find(ep => 
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  (p as any).player_id === (ep as any).player_id
                );
                return enriched || p;
              });
            } catch (err) {
              // If enrichment fails, use players as-is (with ally_code 0)
              players = bracketMeta.bracket_players;
              logger.debug('Ally code enrichment failed for autocomplete, using player IDs');
            }
          } else {
            players = bracketMeta.bracket_players;
          }
          
          // Try quick refresh for real-time data (with short timeout)
          try {
            const liveData = await Promise.race([
              this.comlinkClient.getLiveBracketData(
              ...
              new Promise<never>((_, reject) =>
                setTimeout(() => reject(new Error('Refresh timeout')), 400) // 400ms timeout for refresh
              )
            ]);

            players = this.mergeComlinkBracketData(players, liveData);
            isRealTime = true;
          } catch (refreshError) {
            // Refresh timed out or failed - use discovered bracket data as-is
            logger.debug('Comlink refresh timed out for autocomplete, using discovered data');
          }
        }
      } catch (error) {
        logger.debug('Comlink discovery failed for autocomplete, falling back to swgoh.gg:', error);
      }
    }

    // Step 2: Fall back to swgoh.gg if Comlink didn't work
    if (!bracketMeta) {
      try {
    ...
    // Skip Top 80 GP calculation for autocomplete (not needed)
  ...
  async getLiveBracketWithOpponent(allyCode: string, skipTop80GP: boolean = false): Promise<LiveBracketData> {
    const normalizedAllyCode = allyCode.replace(/-/g, '');
    ...
    // Skip this expensive operation for lightweight fetches (e.g., autocomplete)
    const currentRound = this.calculateCurrentRound(bracketMeta.start_time);
    if (!skipTop80GP && currentRound === 1 && await this.isComlinkAvailable()) {
    ...
    // DISABLED: Automatic opponent matching is unreliable. Users must explicitly select
    // an opponent using the 'allycode' or 'bracket_opponent' options.
    ...
    const opponentConfidence: 'high' | 'medium' | 'low' = 'low';
  ...
  /**
   * Lightweight bracket discovery for autocomplete - skips ally code enrichment.
   * Returns players with temporary ally_code 0 (will be enriched later if needed).
   */
  private async discoverBracketViaComlinkLightweight(allyCode: string): Promise<GacBracketData | null> {
    try {
      ...
      // Convert Comlink bracket players to GacBracketPlayer format WITHOUT fetching ally codes
      // This is much faster for autocomplete - we only need names and IDs
      const bracketPlayers: GacBracketPlayer[] = bracketResult.players.map((cp) => ({
        ally_code: 0, // Will be enriched later if needed
        player_id: cp.id,
        player_name: cp.name,
        player_level: cp.level || 85,
        player_skill_rating: null, // Would need to fetch player data
        player_gp: cp.power,
        top80_character_gp: undefined, // Not needed for autocomplete
      ...
      logger.warn('Failed to discover bracket via Comlink (lightweight):', error);
      return null;
    }
  }
